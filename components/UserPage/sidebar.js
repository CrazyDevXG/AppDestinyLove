                ...
                <div id="sidebar__inner" className="flex sside md:flex-col justify-between md:h-screen md:p-2 p-1 transition-all duration-500 bg-white shadow dark:bg-dark2 2xl:w-72 xl:w-60 max-xl:w-[73px] max-md:w-screen max-md:border-t max-md:dark:border-slate-700">
                    ...
                    <div className="flex h-20 px-2 max-md:fixed max-md:top-0 max-md:w-full max-md:bg-white/80 max-md:left-0 max-md:px-4 max-md:h-14 max-md:shadow-sm max-md:dark:bg-slate-900/80 backdrop-blur-xl">
                        <Link href="/user" id="logo" className="flex items-center gap-3">
                            ...
                            <Image id="logo__img" src={LogoDark} alt="Logo" className="w-full h-10 ml-1 max-xl:hidden max-md:block dark:!hidden" />
                            <Image id="logo__img" src={LogoLight} alt="Logo" className="w-full h-10 ml-1 !hidden max-xl:!hidden max-md:block dark:max-md:!block dark:!block" />
                        </Link>
                    </div>
                    ...
                    <nav className="flex-1 max-md:flex max-md:justify-around md:space-y-2">
                        ...
                        <Link href={navURL("user/matching")}>                            
                        ...
                        <Link href={navURL("user/messages")}>
                        ...
                        <Link href={navURL('user/shop')} className="max-md:!hidden">
                    ...
                    <div >
                        <Link href="" id="profile-link" className="flex items-center gap-3 p-3 group" aria-selected="false">
                        ...
                        <div className="bg-white sm:w-64 2xl:w-[calc(100%-16px)] w-full shadow-lg border rounded-xl overflow-hidden max-md:!top-auto max-md:bottom-16 border2 dark:bg-dark2 hidden" uk-drop="animation:uk-animation-slide-bottom-medium ;animate-out: true">
                            ...
                            <div className="p-4 text-xs font-medium">
                                ...
                                <div className="mt-3 flex gap-3.5">
                                    <div> <Link href={navURL('user/profile')}> <strong> 620K </strong> <span className="text-gray-400 dark:text-white/80 ml-1">Following </span> </Link> </div>
                                    <div> <Link href={navURL('user/profile')}> <strong> 38k </strong> <span className="text-gray-400 dark:text-white/80 ml-1">Followers </span> </Link>  </div>
                                </div>

                            </div>
                            <hr className="opacity-60" />
                            <ul className="text-sm font-semibold p-2">
                                <li> <Link href={navURL('user/profile')} className="flex gap-3 rounded-md p-2 hover:bg-secondery"> <ion-icon name="person-outline" class="text-lg"></ion-icon> My Profile </Link></li>
                                <li> <Link href={navURL("user/upgrade")} className="flex gap-3 rounded-md p-2 hover:bg-secondery"> <ion-icon name="bookmark-outline" class="text-lg"></ion-icon> Upgrade </Link></li>
                                <li> <Link href={navURL("user/setting")} className="flex gap-3 rounded-md p-2 hover:bg-secondery"> <ion-icon name="settings-outline" class="text-lg"></ion-icon> Acount Setting  </Link></li>
                                <li> <Link href={navURL("user/sign_out")} className="flex gap-3 rounded-md p-2 hover:bg-secondery"> <ion-icon name="log-out-outline" class="text-lg"></ion-icon> Log Out </Link></li>
                            </ul>
